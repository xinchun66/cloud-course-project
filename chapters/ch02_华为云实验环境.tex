...
图 \ref{fig:env-swr-images} 中可以看到，郑瑞璟的 SWR 组织下已经有 \texttt{backend} 和 \texttt{frontend} 两个镜像，标签均为 \texttt{v1}。后续 CCE 部署引用的就是这里的镜像地址，而不是临时占位配置；如果私有镜像拉取失败，则需要在 Deployment 中配合 \texttt{imagePullSecrets} 处理鉴权问题。
...
图 \ref{fig:t2-vpc-list} 中，\texttt{cloud-course-vpc} 已处于可用状态，名称也与本报告记录一致。
...
图 \ref{fig:t2-cce-basic-summary} 记录了 CCE 基础配置概要，主要用于核对集群名称、类型和 Kubernetes 版本等参数。
