...
与后端、Redis 不同，当前 \verb|docker-compose.yml| 仅编排 backend 与 redis 两个服务，前端镜像需单独构建并以独立容器运行。本小组在本地执行 \verb|docker build -t frontend:v1 ./frontend|，将 Nginx 配置与 \verb|static/index.html| 一并打入镜像；构建日志中出现 \verb|naming to docker.io/library/frontend:v1| 即表明静态页面已被复制到镜像内的 Nginx 默认网页目录。
