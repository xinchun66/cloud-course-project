...
与后端、Redis 不同，当前 \verb|docker-compose.yml| 仅编排 backend 与 redis 两个服务，前端镜像需单独构建并以独立容器运行。成员 B 在本地执行 \verb|docker build -t frontend:v1 ./frontend|，将 Nginx 配置与 \verb|static/index.html| 一并打入镜像；构建日志中出现 \verb|naming to docker.io/library/frontend:v1| 即表明静态页面已被复制到镜像内的 Nginx 默认网页目录。

\reportfig{figures_B/T1/t1-15-frontend-build-success.png}{本地构建 frontend:v1 镜像成功}{fig:t1-frontend-build}

图 \ref{fig:t1-frontend-build} 记录了前端镜像构建过程。输出中 \verb|COPY static/ /usr/share/nginx/html/| 步骤命中缓存，说明 \verb|index.html| 与静态资源已随镜像分层固化；镜像最终标记为 \verb|frontend:v1|，与后续 SWR 推送及 CCE 中 frontend Deployment 引用的 Tag 保持一致。该步骤从“源码修改”过渡到“可运行、可分发镜像”，是 T1 容器化验收的前置环节。

随后通过 \verb|docker run -d -p 8080:80 --name frontend-demo frontend:v1| 将前端容器映射至本地 8080 端口，在浏览器访问 \verb|http://localhost:8080| 即可验证 Nginx 是否正确提供默认首页，而无需依赖 Kubernetes 或 ELB。

\reportfig[0.72\textwidth]{figures_B/T1/t1-18-frontend-page-student-id.png}{浏览器访问前端首页显示姓名与学号}{fig:t1-frontend-page-student}

图 \ref{fig:t1-frontend-page-student} 展示了前端容器运行后的实际页面效果：标题为 \verb|Cloud Computing Course Project|，正文明确显示 \verb|Name: 李欣纯| 与 \verb|Student ID: 2023116052|。该截图直接对应任务书“在 Nginx 默认首页中加入本人学号和姓名”的硬性要求，并与 \verb|frontend/static/index.html| 源码内容相互印证，避免仅提交镜像 Tag 或 SWR 列表而无法证明页面个性化已完成。
...
2026 年 6 月 17 日，在 Docker Desktop 引擎就绪后，成员 B 按上述流程完整复验一遍。执行 \verb|docker compose up -d --build| 时，后端镜像 \verb|cloud-course-project-backend| 基于多阶段 Dockerfile 完成构建（多数层命中缓存），随后 backend 与 redis 两个容器相继进入 \verb|Started| 状态；终端末尾 \verb|[+] up 3/3| 与两行 \verb|Started| 提示表明 Compose 编排目标已全部达成。
...
本地联调通过后，将后端和前端镜像分别标记为 \verb|v1|，并推送至华为云 SWR。Compose 构建产生的后端镜像默认名为 \verb|cloud-course-project-backend:latest|，推送前需显式 \verb|docker tag| 为 \verb|backend:v1| 及 SWR 完整路径；前端则在图 \ref{fig:t1-frontend-build} 中已直接构建为 \verb|frontend:v1|。成员 B 使用 \verb|docker images| 核对本地镜像清单，确认 backend、frontend 与 redis 基础镜像均已就绪，再执行 tag 与 push。
...
为解决 Pending 问题，在华为云控制台手动创建共享型公网 ELB，并将负载均衡器绑定至 Service。图 \ref{fig:t3-elb-create} 与图 \ref{fig:t3-elb-running} 记录了 ELB 创建配置与运行中状态，与第五章 LoadBalancer Pending 故障的解决路径相对应。

\reportfig{figures_A/T3/13_ELB创建配置概要.png}{手动创建共享型公网 ELB 的配置概要}{fig:t3-elb-create}
