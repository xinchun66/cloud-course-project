...
完成本项目需要把几类课堂知识串起来使用：先根据任务书模板修改 Dockerfile 和应用配置，完成本地联调与镜像推送；再在华为云 CCE 中创建 Kubernetes 集群，用 kubectl 检查节点和资源状态；随后使用 ConfigMap、Secret、PVC、LoadBalancer 和 HPA 处理配置、密码、存储、公网访问和弹性伸缩问题。Spark 部分则要求在 Kubernetes 环境中提交作业，并结合 DataFrame、SQL 查询、运行日志和性能数据解释实验结果，而不是只保留命令截图。
...
报告截图主要来自 `figures` 和 `figures\_A` 目录。`figures/T1` 保存应用容器化、本地联调和 SWR 镜像推送相关截图；`figures\_A/T2` 至 `figures\_A/T5` 保存 CCE 集群、应用部署、Redis 持久化和 ConfigMap Volume 挂载相关截图。后续章节会按任务书验收点插入截图，并说明截图中需要关注的信息，例如 Pod 状态、Service 类型、ELB 公网 IP、PVC Bound 状态、Redis 数据持久化结果以及接口返回值等。这样整理后，正文、代码文件和截图材料可以互相对上，答辩时也更容易查找。
